...
def imfuse(img1, img2):
    ...
    img_fuse = np.dstack((im_cv_1, im_cv_2, im_cv_1))#按照深度堆叠
    return img_fuse

def get_random_affine_coeff():
    # TODO: define transformation coefficent
    degree = random.uniform(0, 10) # light_defo: (0, 0); middle_defo: (0, 5); heavy_defo: (0, 10) 旋转角度
    translate = random.uniform(0.02, 0.05) # light_defo: (0.0, 0.01); middle_defo: (0.0, 0.02); heavy_defo: (0.02, 0.05) 平移
    scale = random.uniform(1.0, 1.0)# 缩放
    shear = None
    return degree, translate, scale, shear

def get_random_deformable_coeff():
    # kernel = random.randrange(63, 101, 2)#随机生成一个奇数，范围是 63 到 101，步长为 2，表示高斯核的大小。这个核的大小决定了变形的平滑程度，较大的值会导致更多的变形。
    kernel = random.randrange(101, 201, 2) #这个调大会有更大的形变，但是范围必须是奇数

    sigma = random.randint(16, 24) # light_defo: (32, 32); middle_defo: (16, 36); heavy_defo: (16, 24) 随机生成一个整数，范围是 16 到 24，表示高斯函数的标准差。sigma 控制着高斯核的宽度，进而影响变形的强度。
    # sigma = random.randint(16, 24)#这个调小会形变更大
    alpha = random.uniform(1.0, 1.2)#alpha: 随机生成一个浮动值，范围在 1.0 到 1.2 之间，表示可变形变换的强度。alpha 控制变形的幅度，1.0 表示没有变形，值越大，变形越强。
    # alpha = random.uniform(1.2, 2.0) #增大会有更大的形变
